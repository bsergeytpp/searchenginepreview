...
// @documentation   http://gwendragon.de/tools/opera/userjs/#searchenginepreview
// @documentation   http://gwendragon.de/tools/opera/userjs/#searchenginepreview
...
var ENABLE_IMAGE_INSERT, ENABLE_PREVIEW_ICON, thumbService;
...
var isGoogle = function (href) {
	return (href.indexOf('.google.') == -1
		 || href.indexOf('news.google.') >= 0
		 || href.indexOf('blogsearch.google.') >= 0
		 || href.indexOf('images.google.') >= 0) ? false : true;
}
...
var isForestle = function (href) {
	return (href.match(/http:\/\/.*forestle\.org\/search/i)
		 || href.match(/http:\/\/.*ecosia\d*\.org\/search/i));
}

var isExcite = function (href) {
	return href.match(/http:\/\/.*\.excite\../i);
}
...
var getImageURL = function (href, psiz) {
	...
	case '1':
	default:
		preview = thumbServiceUrl[1] + site;
		break;
	}
...
var amazonifiy = function () {
	...
	for (i = 0; i < allLinks.length; i++) {
		...
		if (lowerURL.indexOf('tag=') > 0
			 || lowerURL.indexOf('tag%3d') > 0)
			continue;
...
function fetchOptions() {
	var pref = null;
	var ob = {};
	ob["method"] = "getLocalStorage";
	ob["key"] = "previewpic";
	chrome.extension.sendMessage(ob, function (response) {
		pref = response.data;
		ENABLE_PREVIEW_ICON = (pref == 1) || 0;
		ENABLE_IMAGE_INSERT = (pref == 0) || !ENABLE_PREVIEW_ICON;
	});
	ob["method"] = "getLocalStorage";
	ob["key"] = "previewurl";
	chrome.extension.sendMessage(ob, function (response) {
		thumbService = response.data;
	});
}
